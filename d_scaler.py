...
def main():
    ...
    while True:
        MAX_CPU = 0.7
        payload = {'query': "rate(node_pressure_cpu_waiting_seconds_total{instance='172.16.52.8:9100'}[60s])", 'start': (datetime.now()-timedelta(seconds=10)).timestamp(), 'end': datetime.now().timestamp(), 'step': '1s'}
        payload_mem = {'query': "rate(node_pressure_memory_waiting_seconds_total{instance='172.16.52.8:9100'}[60s])", 'start': (datetime.now()-timedelta(seconds=10)).timestamp(), 'end': datetime.now().timestamp(), 'step': '1s'}
        ...
        for dep_name in DEPLOYMENTS:
            ...
            # if mem_pressure > 0:
            #     new_pods = n_pods/2
            # if mem_pressure>0 and n_pods < new_pods:
            #         print("No upscaling")
            #         continue

            if n_pods!=new_pods: #avg_cpu >= MAX_CPU:
                deployment = apps_v1.read_namespaced_deployment(name=dep_name, namespace='default')
                deployment_pers = apps_v1.read_namespaced_deployment(name="teastore-persistence", namespace='default')
                deployment_auth = apps_v1.read_namespaced_deployment(name="teastore-auth", namespace='default')
                deployment_img = apps_v1.read_namespaced_deployment(name="teastore-image", namespace='default')
                # deployment_rec = apps_v1.read_namespaced_deployment(name="teastore-recommender", namespace='default')
                # import ipdb
                # ipdb.set_trace()
                print("scale ", dep_name)
                if new_pods < 1 and n_pods!=1:
                    update_deployment(apps_v1, deployment, 1, dep_name)
                    update_deployment(apps_v1, deployment_pers, 1, "teastore-persistence")
                    update_deployment(apps_v1, deployment_auth, 1, "teastore-auth")
                    update_deployment(apps_v1, deployment_img, 1, "teastore-image")
                    # update_deployment(apps_v1, deployment_rec, 1, "teastore-recommender")
                # elif new_pods > n_pods:
                #     update_deployment(apps_v1, deployment, new_pods, dep_name)
                    ...
                    #if True:
                        update_deployment(apps_v1, deployment, new_pods, dep_name)
                        update_deployment(apps_v1, deployment_pers, new_pods, "teastore-persistence")
                        update_deployment(apps_v1, deployment_auth, new_pods, "teastore-auth")
                        update_deployment(apps_v1, deployment_img, new_pods, "teastore-image")
                        # update_deployment(apps_v1, deployment_rec, new_pods, "teastore-recommender")
